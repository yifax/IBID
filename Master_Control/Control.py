...
def bluet(message):
    ...
    if (message == 1) :
        ...
        print("Speed command sent: run")

    elif (message == 0) :
        port.write(bytes('s','utf-8'))
        print("Speed command sent: stop")

    elif (message == 4) :
        port.write(bytes('b','utf-8'))
        print("Speed command sent: back")
    
    elif (message == 2) :
        port.write(bytes('r','utf-8'))
        print("Speed command sent: turn left")
    
    elif (message == 3) :
        port.write(bytes('l','utf-8'))
        print("Speed command sent: turn right")
...
def s_forward():
    dw = get_ultra(2)
    df = get_ultra(3)
    if ((dw > df) and ((dw-df) > (inital_diff+3))):
        turn_right(0.1)
        forward()
        sleep(0.1)
    elif ((df > dw) and ((df-dw) > (inital_diff+3))):
        turn_left(0.1)
        forward()
        sleep(0.1)
    else:
